...
export default function LoginPage() {
  ...
  const handleLogin = async () => {
    setError("");
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL ?? 'https://hivibe.cloud'}/api/users/login`, {
        method: "POST",
  ...
  return (
    <div className="font-ko min-h-screen bg-background text-foreground flex flex-col">
      ...
      <nav className="h-14 flex items-center px-6 border-b border-border">
        ...
        <span className="font-syne text-lg font-bold mx-auto" style={{ color: BRAND }}>
          HiVibe
        </span>
        <div className="w-20" />
      </nav>
      ...
      <div className="flex-1 flex items-center justify-center px-4 py-12">
        <div className="w-full max-w-[400px]">
          <div className="text-center mb-8">
            <p className="font-ko text-[10px] tracking-widest mb-3" style={{ color: BRAND }}>
              // WELCOME BACK
            </p>
            <h1 className="font-syne text-3xl font-bold text-foreground mb-2">
              로그인
            </h1>
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-1.5">
                <div className="flex items-center justify-between">
                  ...
                  <Link href="#" className="font-ko text-[10px] hover:underline" style={{ color: BRAND }}>
                    비밀번호 찾기
                  </Link>
                </div>
                <div className="relative">
                  <Input
                    ...
                    className="h-11 bg-muted border-border font-ko text-sm pr-10 focus-visible:ring-1"
                    style={{ ["--ring" as string]: BRAND }}
                  />
                  <button
                    type="button"
          ...
          <p className="text-center font-ko text-sm text-muted-foreground mt-6">
            계정이 없으신가요?{" "}
            <Link href="/signup" className="font-semibold hover:underline" style={{ color: BRAND }}>
              회원가입
            </Link>
          </p>
        </div>
