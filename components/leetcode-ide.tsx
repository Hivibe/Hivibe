...
import {
  Play, Copy, TrendingUp, Sparkles, FileCode, Bookmark, ChevronDown, Lightbulb,
  X, Timer, PanelLeft, Home, Activity as ActivityIcon, Book, Settings, Search,
  Edit2, Share2, Trash2, BookOpen, Code2, GraduationCap, Upload, Save,
...
/* ── BRAND ─────────────────────────────
   Main color : #63C1ED
   Title font : Syne  (font-syne)
   Body/code  : Space Mono (font-space)
──────────────────────────────────────── */
...
const mockNotes = [
  ...
  { id: 2, title: "Binary Search Implementation", date: "Oct 20, 2025", grade: "A",  tags: ["Binary Search","Arrays"],         language: "Java"   },
  { id: 3, title: "Hash Map Collision Resolution", date: "Oct 18, 2025", grade: "A-", tags: ["Hash Table","Collision"],         language: "Java"   },
  { id: 4, title: "Dynamic Programming",           date: "Oct 15, 2025", grade: "B",  tags: ["DP","Memoization"],              language: "Python" },
]
...
export function LeetCodeIDE() {
  ...
  const recentSessions = sessions.filter(s => !s.favorited && (langFilter === "All" || s.language === langFilter))
      ...
      <div className="h-screen w-full bg-zinc-950 flex overflow-hidden">
        ...
        <div className="flex-1 flex flex-col min-w-0">
          ...
          <header className="bg-[#0a0a0a] border-b border-zinc-800/50 shrink-0">
            ...
            <div className="h-14 flex items-center justify-between px-5">
              ...
              {activeNav !== "notes" && (
                <div className="flex items-center gap-2.5">
                  ...
                  {activeNav === "diagnosis" && (
                    <Button size="sm" disabled={!hasAnalyzed} onClick={() => { setAnalyzedCode(editorCode); setActiveNav("learning") }}
                  ...
                  {activeNav === "diagnosis" && (
                    <Button size="sm" disabled={!editorCode.trim()} onClick={() => { if (editorCode.trim()) setHasAnalyzed(true) }}
            ...
            {/* Row 2: filename bar (diagnosis, or learning+session) */}
              ...
              <div className="h-9 flex items-center justify-between px-5 border-t border-zinc-800/40">
                <div className="flex items-center gap-2">
                  ...
                  <input value={fileName} onChange={e => setFileName(e.target.value)} placeholder="Name your file..."
                    className="bg-transparent text-xs outline-none border-none w-44 font-space placeholder:text-zinc-700"
                    style={{ color: "#FAFAFA" }} />
                  <span className="font-space text-xs text-zinc-700">.{ext}</span>
                  {activeNav === "learning" && selSession && (
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <button onClick={() => setSaveNoteOpen(true)} className="h-5 w-5 flex items-center justify-center rounded text-zinc-600 hover:text-zinc-300 hover:bg-zinc-800 transition-colors ml-1">
            ...
            <ResizablePanelGroup direction="horizontal" className="flex-1">
              <ResizablePanel defaultSize={35} minSize={25} maxSize={50} className="bg-[#0a0a0a]">
                <div className="h-full flex flex-col p-5">
                  <h2 className="font-syne text-xl font-bold text-zinc-100 mb-4">My Library</h2>
                  <div className="relative mb-4">
                    <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-zinc-500" />
                    <Input placeholder="Search by keywords or tags..." className="pl-9 bg-zinc-900 border-zinc-800 text-zinc-200 text-xs h-8 font-space" style={{ ["--ring" as string]: BRAND }} />
                  </div>
                  <div className="flex gap-1.5 mb-5 flex-wrap">
                    {["All","Java","Python"].map(l => (
                      <button key={l} className="font-space text-[10px] px-2.5 py-1 rounded-full border transition-all"
                        style={l==="All" ? { background:`${BRAND}15`, color:BRAND, borderColor:`${BRAND}44` } : { color:"#71717a", borderColor:"#27272a" }}>
                        {l}
                      </button>
                    ))}
                  </div>
                  <ScrollArea className="flex-1">
                    <div className="space-y-2.5 pb-4">
                      {mockNotes.map(note => (
                        <div key={note.id} onClick={() => setSelNote(note.id)}
                          className="p-3.5 rounded-xl border cursor-pointer transition-all"
                          style={selNote === note.id ? { borderColor:`${BRAND}44`, background:`${BRAND}08` } : { borderColor:"#27272a", background:"#18181b55" }}>
                          <div className="flex justify-between items-start mb-1">
                            <h3 className="font-syne text-xs font-semibold text-zinc-100 leading-snug">{note.title}</h3>
                            <span className="font-space text-[10px] px-1.5 py-0.5 rounded bg-zinc-800 text-zinc-300 ml-2 shrink-0">{note.grade}</span>
                          </div>
                          <p className="font-space text-[10px] text-zinc-600 mb-2">{note.date}</p>
                          <div className="flex flex-wrap gap-1">
                            {note.tags.map(t => <span key={t} className="font-space text-[9px] px-1.5 py-0.5 rounded-full border border-zinc-800 text-zinc-500">#{t}</span>)}
                          </div>
                        </div>
                      ))}
                    </div>
              ...
              <ResizablePanel defaultSize={65} className="bg-zinc-950">
                {(() => {
                  const note = mockNotes.find(n => n.id === selNote) ?? mockNotes[0]
                  return (
                    <ScrollArea className="h-full">
                      <div className="p-8 max-w-3xl mx-auto space-y-5">
                        <div className="flex justify-between items-start">
                          <div>
                            <h1 className="font-syne text-2xl font-bold text-zinc-100">{note.title}</h1>
                            <p className="font-space text-xs text-zinc-500 mt-1">{note.date}</p>
                          </div>
                          <div className="flex gap-2">
                            <Button size="sm" variant="outline" className="h-7 border-zinc-800 bg-zinc-900 text-zinc-400 text-xs"><Edit2 className="h-3 w-3 mr-1" />Edit</Button>
                            <Button size="sm" variant="outline" className="h-7 border-zinc-800 bg-zinc-900 text-zinc-400 text-xs"><Share2 className="h-3 w-3 mr-1" />Share</Button>
                            <Button size="sm" variant="outline" className="h-7 w-7 p-0 border-rose-900/50 bg-rose-500/10 text-rose-500"><Trash2 className="h-3.5 w-3.5" /></Button>
                          </div>
                        </div>
                        <Card className="bg-zinc-900/50 border-zinc-800">
                          <CardHeader className="pb-2 pt-4 px-4">
                            <CardTitle className="font-space text-[10px] tracking-wider flex items-center gap-2" style={{ color: BRAND }}>
                              <BookOpen className="h-3.5 w-3.5" />PERSONAL NOTES
                            </CardTitle>
                          </CardHeader>
                          <CardContent className="px-4 pb-4">
                            <p className="font-space text-xs text-zinc-300 leading-relaxed">Applied memoization to avoid redundant traversals. Time reduced from O(V+E) per query to O(V+E) amortized.</p>
                          </CardContent>
                        </Card>
                        <Card className="bg-zinc-900/50 border-zinc-800">
                          <CardHeader className="pb-2 pt-4 px-4 flex flex-row items-center justify-between">
                            <CardTitle className="font-space text-[10px] tracking-wider text-zinc-400 flex items-center gap-2"><Code2 className="h-3.5 w-3.5" />CODE SNAPSHOT</CardTitle>
                            <span className="font-space text-[10px] px-2 py-0.5 rounded border border-zinc-700 text-zinc-400">{note.language}</span>
                          </CardHeader>
                          <CardContent className="px-4 pb-4">
                            <div className="rounded-lg overflow-hidden bg-[#141414] border border-zinc-800 font-space text-[12px]">
                              <div className="flex">
                                <div className="px-3 py-3 text-zinc-700 text-right select-none border-r border-zinc-800 min-w-[36px]">
                                  {Array.from({length:8},(_,i)=><div key={i} className="leading-6">{i+1}</div>)}
                                </div>
                                <pre className="flex-1 px-4 py-3 text-zinc-300 leading-6 overflow-x-auto"><code>
                                  <span className="text-purple-400">def</span> <span style={{color:BRAND}}>dfs</span>(graph, start):{"\n"}
                                  {"    "}visited = <span className="text-blue-400">set</span>(){"\n"}
                                  {"    "}<span className="text-purple-400">def</span> <span style={{color:BRAND}}>_dfs</span>(node):{"\n"}
                                  {"        "}visited.<span className="text-amber-400">add</span>(node){"\n"}
                                  {"        "}<span className="text-purple-400">for</span> n <span className="text-purple-400">in</span> graph[node]:{"\n"}
                                  {"            "}<span className="text-purple-400">if</span> n <span className="text-purple-400">not in</span> visited: <span style={{color:BRAND}}>_dfs</span>(n){"\n"}
                                  {"    "}<span style={{color:BRAND}}>_dfs</span>(start); <span className="text-purple-400">return</span> visited
                                </code></pre>
                              </div>
                            </div>
                          </CardContent>
              ...
              {!selSession && (
                <div className="flex-1 overflow-auto bg-zinc-950">
                  <div className="max-w-3xl mx-auto px-8 py-8">
                    ...
                    <div className="flex items-center gap-2 mb-7 flex-wrap">
                      {["All","Java","Python","C++"].map(l => (
                        <button key={l} onClick={() => setLangFilter(l)}
                          ...
                          style={langFilter===l ? { background:`${BRAND}15`, color:BRAND, borderColor:`${BRAND}44` } : { color:"#71717a", borderColor:"#27272a" }}>
                          {l}
                        </button>
                      ))}
                      <button onClick={() => setFavOnly(!favOnly)}
                        ...
                        style={favOnly ? { background:"#f59e0b22", color:"#f59e0b", borderColor:"#f59e0b44" } : { color:"#71717a", borderColor:"#27272a" }}>
                    ...
                    <div>
                      <div className="flex items-center gap-2 mb-3">
                        <span className="font-space text-[10px] text-zinc-500 tracking-widest uppercase">Recent Sessions</span>
                        <span className="font-space text-[10px] px-1.5 py-0.5 rounded-full border border-zinc-800 text-zinc-500">{recentSessions.length}</span>
                      </div>
                  ...
                  <ResizablePanel defaultSize={62} minSize={35}>
                    <div className="h-full flex overflow-hidden">
                      ...
                      <div className="flex-1 flex flex-col border-r border-zinc-800">
                        ...
                        <div className="flex-1 overflow-auto bg-[#141414] font-space text-[12px]">
                          <div className="flex">
          ...
          {activeNav === "diagnosis" && (
            <ResizablePanelGroup direction="horizontal" className="flex-1">
                          ...
                          <CardContent className="px-4 pb-4">
                            <div className="flex items-center gap-5">
                              ...
                              <div className="flex-1 space-y-2.5">
                                {[{l:"Accuracy",v:72,c:"bg-amber-500"},{l:"Efficiency",v:28,c:"bg-rose-500"},{l:"Readability",v:65,c:"bg-amber-500"},{l:"Style",v:44,c:"bg-rose-500"}].map(s=>(
                                  <div key={s.l}>
              ...
              <ResizablePanel defaultSize={60} minSize={35}>
                <div className="h-full overflow-auto bg-[#141414] relative">
                  <div className="flex font-space text-[12px]">
                    <div className="sticky left-0 bg-[#141414] border-r border-zinc-800/60 select-none">
                      <div className="px-3 py-4 text-zinc-700 text-right">
                    ...
                    <div className="flex-1 relative">
                      <textarea value={editorCode} onChange={e=>setEditorCode(e.target.value)}
                        placeholder={"// 여기에 코드를 붙여넣으세요..."} spellCheck={false}
                        className="w-full h-full min-h-[calc(100vh-10rem)] px-4 py-4 bg-transparent text-zinc-300 font-space text-[12px] leading-6 resize-none outline-none border-none placeholder:text-zinc-700"
                        style={{ caretColor: BRAND }} />
                      {aiCoaching && editorCode.trim() && !hasAnalyzed && (
                        <div className="absolute left-4 bottom-8 z-10 animate-in fade-in slide-in-from-bottom-2 duration-300">
          ...
          <DialogContent className="bg-zinc-900 border-zinc-800 text-zinc-100 sm:max-w-md">
            ...
            <div className="space-y-4 py-2">
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  <Input value={tagInput} onChange={e=>setTagInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&addTag()} placeholder="태그 입력 후 Enter..." className="bg-zinc-950 border-zinc-800 text-zinc-200 text-sm font-space" />
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex flex-wrap gap-1 mt-2">
            {s.tags.slice(0, compact ? 2 : 3).map(t=>(
              <span key={t} className="font-space text-[9px] px-1.5 py-0.5 rounded-full border border-zinc-800 text-zinc-500">#{t}</span>
            ))}
          </div>
        </div>
